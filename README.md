...
Here are all the available commands for your bot:

- **`/start`** - Start the bot and see welcome message
- **`/status`** - View your current configuration (time, bus ID, token status)
- **`/get`** - Automatically retrieve le_token from intra login ⭐
- **`/bus`** - Show currently available buses with their IDs
- **`/next`** - Show upcoming buses in the next hour with predicted IDs 🔮
- **`/run`** - Execute the booking command at scheduled time
- **`/cancel`** - Cancel any scheduled booking command
...
2. **Check upcoming buses in the next hour:**
   ```
   /next
   ```
   This shows upcoming buses with predicted IDs based on current patterns.

3. **Set the time** you want to book (must be in the future):
   ```
   time: 15:10:22
   ```
   Format: `HH:MM:SS` (24-hour format)

4. **Set the bus ID** you want to book:
   ```
   id: 18399
   ```
   Use the ID you got from the `/bus` or `/next` command.

5. **Schedule the booking:**
   ```
   /run
   ```
...
3. Send: /next
   Bot: 🕐 Buses available in the next hour:
        ...
        💡 Tip: Use id: 18650 to set your bus ID

4. Send: time: 15:10:22
   Bot: 🕐 Time set to 15:10:22

5. Send: id: 18650
   Bot: 🚌 Bus ID set to 18650
...
- Use `/next` to see predicted IDs for upcoming buses in the next hour
...
### Bus ID Prediction
- `/next` command shows upcoming buses in the next hour
- Predicts sequential IDs based on current bus patterns
- Calculates next IDs (e.g., 18649 → 18650, 18651, etc.)
- Handles multiple buses at the same time slot
- IDs are marked with `~` to indicate they are predictions
- Based on the last available bus ID from `/bus` command
